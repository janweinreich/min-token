...
import { loadCases, runBenchmark } from "../packages/core/src/eval/runner.js";
...
const evaluate: EvaluateFn = async (policy, setName) => {
  ...
  for (const e of r.episodes) {
    ...
    collected.push({ ...e, taskType: e.taskType ?? c?.taskType ?? "unknown", generationTokens: 0 });
  }
  return { results: r.results, replayCorrect: r.replayCorrect, replayTotal: r.replayTotal };
};
...
for (const e of episodes) collected.push({ ...e, taskType: e.taskType ?? "unknown", generationTokens: 0 });
