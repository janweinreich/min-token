...
export function MintokenApp({
  ...
  const selectChat = (chatId: string) => {
    ...
    setState((s) => ({ ...s, active_chat_id: chatId }));
  };
    ...
    setState((s) => ({
      ...
      events: [
        ...s.events,
        {
          ...
          source: "engine",
          summary: "Replay QA marked (shell only).",
        },
      ],
    }));
  };
  ...
  const submit = async () => {
    ...
    // Local shell demo: append messages only (no model / sponsor calls).
    setPending(true);
    const at = new Date().toISOString();
    const userId = `msg-${Math.random().toString(36).slice(2, 9)}`;
    const asstId = `msg-${Math.random().toString(36).slice(2, 9)}`;

    setState((s) => {
      const chats = s.chats.map((c) => {
        if (c.id !== s.active_chat_id) return c;
        const firstUser = c.messages.every((m) => m.role !== "user");
        return {
          ...c,
          title: firstUser ? titleFrom(q) : c.title,
          messages: [
            ...c.messages,
            { id: userId, role: "user" as const, content: q, at },
            {
              id: asstId,
              role: "assistant" as const,
              content:
                "UI shell placeholder. Connect Pioneer routing, Senso scoring, Guild promote, Band announce, and answer memory to this composer.",
              at,
              tier: "mid",
              model: "shell/placeholder",
              quality: 0.92,
              cost_usd: 0,
              from_memory: false,
            },
      ...
      return {
        ...
        events: [
          ...s.events,
          {
            id: `evt-${Date.now()}`,
            source: "engine",
            summary: `Shell received prompt (${q.slice(0, 48)}${q.length > 48 ? "..." : ""}).`,
          },
        ],
      };
    });
      ...
      <div className="main-col">
        <section className="dashboard" aria-label="mintoken dashboard">
          ...
          <div className="workspace">
            <p className="lede">
              Ask anything in the composer. Each prompt will route through Pioneer
              (cheap / mid / premium), score with Senso, and can promote a cheaper
              policy via Guild once wired.
            </p>
              ...
              <div className="left-stack">
                <section className="panel chat-panel">
                  ...
                  <div className="panel-b chat-thread">
                    ...
                    {pending ? (
                      <p className="msg-pending">Waiting on wired handler…</p>
                    ) : null}
                    <div ref={threadEnd} />
                  </div>
                </section>
                ...
                <section className="panel policy-panel">
                  ...
                  <div className="panel-b">
                    ...
                    {state.generations.length === 0 ? (
                      <p className="empty">
                        Generations appear here once the loop is wired.
                      </p>
                    ) : (
                ...
                <section className="panel memory-panel">
                  ...
                  <div className="panel-b">
                    {state.memory.length === 0 ? (
                      <p className="empty">Empty until memory is wired.</p>
                    ) : (
                      <table className="mem">
                        <thead>
                          <tr>
        ...
        >
          ...
          <p className="composer-hint">
            Enter to send · Shift+Enter for newline · UI shell (wire handlers to
            go live)
          </p>
        </form>
      </div>
